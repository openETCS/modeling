...
\textbf{TODO:} How to handle the Connection status information? Forward to RBC session management? Could also be stored in the database. Suggestion: Since the architecture is defined like in Jakobs slides, the solution should be to also execute the \texttt{WRITE\_RTM\_MESSAGE} service in the Manage Radio communication module in order to get status information.

\textbf{TODO:} Structure of RTM\_IN\_MESSAGE\_T
...
The module produces the input-data for the Filtering (Mode/Level) module. The Filtering (Mode/Level)-module will need the messages received either via Euroradio or Eurobalise. The message is transmitted unchanged, if all consistency checks were successful.

An Euroradio message from track to train consists of several fields shown in the table below. The table is an excerpt from the SRS, chapter 8.4.4.6.1. In this chapter the description of the fields can be found. The length of the fields is derived from chapter 7.
...
\begin{itemize}
 \item check length of message (SRS 8.4.4.2.1)
 \item does the message contain all necessary fields and no invalid values in the fields? (invalid value = spare value (SRS 3.16.1.1.1)) (SRS 3.16.1.1)
 \item check value range of fields
 \item was the message received in due time? (SRS 3.16.1.1) / timestamp check (SRS 3.16.3.3.3)
 \item was the message received at the right expected location? (SRS 3.16.1.1)
 \item if a message is not consistent, it will be rejected. (SRS 3.16.3.1.1.1)
 \item RBC must be informed, when a message was rejected (SRS 3.16.3.1.1.2) $\rightarrow$ there must be a direct possibility to send this status message, which is not possible in the current architecture.
 \item check priority of message (SRS 3.16.3.1.2 / 3.16.3.1.3 / 3.16.3.1.3.1)
 \item some checks shall not be performed when a high priority message is received (SRS 3.16.3.1.4)
 \item Safe connection supervision is not in the scope of this module. Will be done by the RBC session management module.
 \item Send ACK for received messages (3.16.3.5)
 \item supervise sequence (SRS 3.16.3.3)
\end{itemize}
