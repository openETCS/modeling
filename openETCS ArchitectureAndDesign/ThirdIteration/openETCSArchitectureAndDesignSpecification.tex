...
\usepackage[modulo]{lineno}
 ...
 style=longheaderborder, % table style
...
\begin{itemize}
\item \gls{EVC} (European Vital Computer);
\item \gls{TIU} (Train Interface Unit);
\item \gls{ODO} (Odometry);
\item \gls{DMI} (Driver Machine Interface);
\item \gls{STM} (Specific Transmission Module, up to 8 units): Not part of this openETCS implementation;
\item \gls{BTM} (Balise Transmission Module);
...
% LocalWords:  Alstom openETCS \gls{EVC} BIU TIU Odometry \gls{DMI} \gls{STM} \gls{BTM} Balise LTM \gls{API}
...
- Store inputs from the \gls{DMI}\\
- Filtering (Mode/Level)\\
- Store data (direct orders, \gls{BG} lists, NV, track data, procedure parameters, confirmations)\\
- Update location based  data structures\\
- Build and update MRSP and list of targets at LR\gls{BG}\\
...
- RBC communication\\
- TIU Communication\\
- \gls{STM} management (not taken into account)\\
- Gathering JRU data (not taken into account)\\

\section{second level functional block}

\subsection{F1.1: openETCS \gls{API} Runtime System and Input)}
...
\subsubsection{Input Interfaces of the openETCS \gls{API}}
...
\subsection{F1.2: openETCS \gls{API} Output Functions)}
