...
\begin{table}[H]
  \begin{tabular}{| c | l | l | l |}
    ...
    1 & \texttt{Time} & CLOCK\_T\\
    2 & \texttt{T\_NVContact} & T\_NVCONTACT\\
    3 & \texttt{Reset} & Boolean \\
    \hline
  \end{tabular} 
...
The Euroradio-/Eurobalise-message is originated from the openETCS-API. The API is described in \cite{alstom-api}.
...
In the model, the output \texttt{THE\_RTM\_MESSAGE} of the API will be received at the input \texttt{TheRtmMessage}.
...
\paragraph{Input 1: \texttt{Time}}

The current time of the OBU is needed as an input to perform consistency checks. Therefore, the current time of the OBU is provided to the module as \texttt{CLOCK\_T}-type.

\paragraph{Input 2: \texttt{T\_NVContact}}

For checking the timing of the messages, also the national value \texttt{T\_NVCONTACT} is needed.
...
\begin{table}[H]
  \begin{tabular}{| c | l | l | l |}
    ...
    0 & \texttt{RadioMessage} & EuroradioMessage \\
    1 & \texttt{MessageToRbc} & < undefined >\\
    \hline
  \end{tabular} 
...
\paragraph{Output 0: \texttt{RadioMessage}}
...
\paragraph{Output 1: \texttt{MessageToRbc}}
\textbf{TODO}
