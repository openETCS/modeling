...
TIME & Time system of the OBU & 1 & All Parts & \\\hline
Startup \\\hline
...
\hline 
\rowcolor{gray} 
Interface & Definition & Remarks \\\hline}
\begin{supertabular}{| p{2 cm} | p{3 cm} | p{10 cm} |}
TIME & T\_internal\_Type\newline (32-bit INT) & Standardized system time type used for all internal time calculations: in ms. The time is defined as a cyclic counter: When the maximum is exceeded the time starts from 0 again. 
\\\hline
\end{supertabular}
...
\hline 
\rowcolor{gray} 
Interface & Logical Interface & Remarks \\\hline}
\begin{supertabular}{| p{2 cm} | p{4 cm} | p{10 cm} |}
Odometer
 & odometry\_T\newline 
(Structure of:) &
\\\hline
& - valid (bool) & valid flag, i.e., the information is provided by the ODO system and can be used.
\\\hline
& - timestamp & time  (T\_internal\_Type))of the system when the odometer information was collected. Please, see also general remarks on the time system. 
\\\hline
& -Coordinate\newline 
(Structure of:) \newline
- nominal \newline
- min  \newline
- max  \newline
 &  odometer values. All locations in cm (internal type (L\_internal\_Type)).
\\\hline
& - speed   & V\_internal\_Type
(General Speed, Int, km/h)
\\\hline
& - acceleration   & A\_internal\_Type, Standardized acceleration type for all internal calculations: in 0.01 m/s2
\\\hline
& - motionState (Enumeration)   & indicates whether the train is in motion or in no motion
\\\hline
& - motionDirection (Enumeration)  & indicates the direction of the train, i.e., CAB-A first, CAB-B first or unknown.
\\\hline
\end{supertabular}


\subsubsection{Interfaces to Odometry}
...
\begin{table}[H]
  \begin{tabular}{| c | l | l | l |}
    ...
    1 & \texttt{apiConsistencyError} & \texttt{bool}\\
    2 & \texttt{lastRelevantEventTimestamp} & \texttt{T\_internal\_Type}\\
    3 & \texttt{t\_NVContact} & \texttt{T\_internal\_Type}\\
    4 & \texttt{reset} & \texttt{bool} \\
    \hline
  \end{tabular} 
...
\paragraph{Input 1: \texttt{apiConsistencyError}}
...
\paragraph{Input 2: \texttt{lastRelevantEventTimestamp}}
...
Since the situation can occur, that a Euroradio-message and a Eurobalise-message are delivered to the output in the same cycle of the module, the output consists of a structure of both data types. Dependant on the input and the processing of the message, the output can contain
\begin{itemize}
 \item \textbf{0} Euroradio-message and \textbf{0} Eurobalise-message
 \item \textbf{1} Euroradio-message and \textbf{0} Eurobalise-message
 \item \textbf{0} Euroradio-message and \textbf{1} Eurobalise-message
 \item \textbf{1} Euroradio-message and \textbf{1} Eurobalise-message
\end{itemize}

To mark, in which elements valid data is available, the elements can be marked with valid-flags (see specification of the datatypes).

\begin{table}[H]
  \begin{tabular}{| c | l | l | l |}
    \hline
    \textbf{Index} & \textbf{Output name} & \textbf{Output type}\\ \hline
    0 & MessageOutputSlot1 & \texttt{checkedMessage\_T}\\
    1 & MessageOutputSlot2 & \texttt{checkedMessage\_T}\\
    \hline
  \end{tabular} 
...
\paragraph{Output 0: \texttt{checkedMessage\_T}}
The type \texttt{checkedMessage\_T} consists of the fields described in table \ref{tbl:checkedMessage}.

\begin{table}[H]
  \begin{tabular}{| c | l | l | l |}
    \hline
    \textbf{Index} & \textbf{Element name} & \textbf{Element type}\\ \hline
    0 & \texttt{valid} & \texttt{bool}\\
    1 & \texttt{rejected} & \texttt{bool}\\
    2 & \texttt{rejectionReason} & Boolean-Array (to be defined)\\
    3 & \texttt{acknowledgementRequired} & \texttt{bool}\\
    4 & \texttt{radioConnectionStatus} & \texttt{Enumeration}\\
...
\subparagraph{Element 0: \texttt{valid}}
The valid-flag specifies, if the data in this element is valid or if it has to be ignored while processing.

\begin{table}[H]
  \begin{tabular}{| l | p{13cm} |}
    \hline
    \textbf{Value} & \textbf{Interpretation}\\ \hline
    false & The data in this element is not valid and has to be ignored. \\
...
\subparagraph{Element 1: \texttt{rejected}}

The output \texttt{rejected} is giving information if the current message was rejected. The value of the flag is the result of the consistency check.

\begin{table}[H]
  \begin{tabular}{| l | p{13cm} |}
    \hline
    \textbf{Value} & \textbf{Interpretation}\\ \hline
    false & The message at output \texttt{receivedMessage} is consistent and was not rejected. \\
    true & During the consistency check, consistency errors were detected and the message was rejected. \\
    \hline
  \end{tabular} 
  \caption{Possible values for the output \texttt{rejected}}
  \label{tbl:Rejected}
\end{table}

\subparagraph{Element 2: \texttt{rejectedReason}}
...
\subparagraph{Element 3: \texttt{acknowledgementRequired}}
...
\subparagraph{Element 4: \texttt{radioConnectionStatus}}
The output ConnectionStatus is used, when the RTM reports problems with the radio connection. The output is derived from the Alstom-API. %\textbf{TODO!} 
...
\subparagraph{Element 5: \texttt{receivedMessage}}
...
\begin{table}[H]
  \begin{tabular}{| c | l | l | l |}
  \hline
  \textbf{Field No.} & \textbf{Name} & \textbf{Datatype} & \textbf{Origin}\\ \hline
  - & nid\_message & NID\_MESSAGE & Euroradio-Header\\
  - & l\_message & L\_MESSAGE & Euroradio-Header\\
...
\framebox{Note: Only radio functionality covered for checks}
