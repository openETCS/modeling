...
void ScanPositions_reset_2_2(outC_ScanPositions_2_2 *outC)
{
  kcg_int i;
  
  for (i = 0; i < 2; i++) {
    /* 1 */ ScanSSP_reset_2(&outC->Context_1[i]);
  }
}

/* ScanPositions */
void ScanPositions_2_2(
  ...
  outC_ScanPositions_2_2 *outC)
{
  ...
  kcg_copy_array__1547(&outC->_L2, SpeedProfile);
  outC->_L28 = 2 > 0;
  outC->_L32 = outC->_L22;
  if (outC->_L28) {
    for (i = 0; i < 2; i++) {
