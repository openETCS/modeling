...
/*
 * NIRT_SetScalarParameterInline
 * Called inline in control loop.
 * 	
 * Inputs:
 ...
 */
DLL_EXPORT long NIRT_SetScalarParameterInline( unsigned long index,  unsigned long subindex, double paramvalue);

/*
 * NIRT_SetVectorParameter
 * Called in background loop. 
 * 	
 * Inputs:
 *		index: index of the parameter (as specified in Parameter Information)
 *		paramvalues: array of values specifying the parameter
 *		paramlength: Length of parameter values. ues to validate if paramvalues is of correct length
 * Return value: 0 if no error
 */
DLL_EXPORT long NIRT_SetVectorParameter( unsigned long index, const double* paramvalues, unsigned long paramlength);

/*
 * NIRT_GetModelFrameworkVersion
 * Returns the version of the NIVS Model Framework with which this model was compiled
 *
 * Outputs
 ...
 *Return value: 0 if no error
 */
DLL_EXPORT long NIRT_GetModelFrameworkVersion(unsigned int* major, unsigned int* minor, unsigned int* fix, unsigned int* build);
 ...
 * Outputs:
 *		paramValues: array of values specfying the parameter
 * Return value: 0 if no error
 */
DLL_EXPORT long NIRT_GetVectorParameter(unsigned long index, double* paramValues, unsigned long paramLength);

/*
 * NIRT_GetErrorMessageLength
 * Gets the length of the error message.
 *
 * Return value: Length of the error message.
 */
