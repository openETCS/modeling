  ...
  outC_Convert_P21_to_DistancePr_TA_Gradient *outC)
{
  ...
  for (i = 0; i < 33; i++) {
    /* 1 */ _1_Convert_P21_to_DistancePr_reset_TA_Gradient(&outC->Context_1[i]);
  }
}
  ...
  outC_Convert_P21_to_DistancePr_TA_Gradient *outC)
{
  ...
  kcg_copy_P021_OBU_T_TM(&outC->_L73, &inC->Packet21_in);
  ...
  kcg_copy_P021_OBU_T_TM(&outC->_L63, &inC->Packet21_in);
  ...
  for (i = 0; i < 33; i++) {
    ...
    /* 1 */
    _1_Convert_P21_to_DistancePr_TA_Gradient(
      i,
      &tmp,
      &outC->_L74[i],
      outC->_L76[i],
