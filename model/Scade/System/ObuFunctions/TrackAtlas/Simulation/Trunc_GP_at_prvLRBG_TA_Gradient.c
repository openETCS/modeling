/* $*************** KCG Version 6.1.3 (build i6) ****************
** Command: s2c613 -config C:/GITHUB/modeling/model/Scade/System/ObuFunctions/TrackAtlas/Simulation\kcg_s2c_config.txt
** Generation date: 2015-08-31T17:27:48
*************************************************************$ */

#include "kcg_consts.h"
#include "kcg_sensors.h"
#include "Trunc_GP_at_prvLRBG_TA_Gradient.h"

void Trunc_GP_at_prvLRBG_reset_TA_Gradient(
  outC_Trunc_GP_at_prvLRBG_TA_Gradient *outC)
{
  kcg_int i;
  
  for (i = 0; i < 33; i++) {
    /* 1 */ Trunc_GP_at_prvLRBG_Abs_Loop_reset_TA_Gradient(&outC->Context_1[i]);
  }
}

/* TA_Gradient::Trunc_GP_at_prvLRBG */
void Trunc_GP_at_prvLRBG_TA_Gradient(
  inC_Trunc_GP_at_prvLRBG_TA_Gradient *inC,
  outC_Trunc_GP_at_prvLRBG_TA_Gradient *outC)
{
  kcg_int i2;
  kcg_int i1;
  kcg_int tmp;
  kcg_int i;
  kcg_int noname;
  
  outC->_L1 = inC->position_BG;
  for (i2 = 0; i2 < 33; i2++) {
    outC->_L170[i2] = outC->_L1;
  }
  kcg_copy_GradientProfile_t_TrackAtlasTypes(&outC->_L93, &inC->Profile_in);
  for (i1 = 0; i1 < 33; i1++) {
    kcg_copy_GradientProfile_t_TrackAtlasTypes(&outC->_L169[i1], &outC->_L93);
  }
  outC->_L100 = 0;
  outC->_L96 = kcg_true;
  outC->_L168 = outC->_L100;
  if (outC->_L96) {
    for (i = 0; i < 33; i++) {
      tmp = outC->_L168;
      /* 1 */
      Trunc_GP_at_prvLRBG_Abs_Loop_TA_Gradient(
        i,
        tmp,
        &outC->_L169[i],
        outC->_L170[i],
        &outC->Context_1[i]);
      outC->_L168 = outC->Context_1[i].relevant_section;
      outC->_L94 = i + 1;
      if (!outC->Context_1[i].cont) {
        break;
      }
    }
  }
  else {
    outC->_L94 = 0;
  }
  noname = outC->_L94;
  outC->relevant_section = outC->_L168;
}

/* $*************** KCG Version 6.1.3 (build i6) ****************
** Trunc_GP_at_prvLRBG_TA_Gradient.c
** Generation date: 2015-08-31T17:27:48
*************************************************************$ */

