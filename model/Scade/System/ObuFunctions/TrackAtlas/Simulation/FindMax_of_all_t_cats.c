...
void FindMax_of_all_t_cats_reset(outC_FindMax_of_all_t_cats *outC)
{
  kcg_int i;
  
  for (i = 0; i < 11; i++) {
    /* 1 */ FindMax_of_all_t_cats_LOOP_reset(&outC->_1_Context_1[i]);
  }
  for (i = 0; i < 11; i++) {
...
{
  kcg_int i1;
  SSP_t_indexed_matrix_element tmp;
  kcg_int i;
  kcg_int noname;
  ...
  kcg_copy_SSP_t_indexed_matrix_element(
    &outC->_L27,
    (SSP_t_indexed_matrix_element *) &DEFAULT_indexed_target);
  outC->_L25 = kcg_true;
  kcg_copy_SSP_t_indexed_matrix_element(&outC->_L26, &outC->_L27);
  if (outC->_L25) {
    for (i = 0; i < 11; i++) {
      kcg_copy_SSP_t_indexed_matrix_element(&tmp, &outC->_L26);
      /* 1 */
  ...
  noname = outC->_L24;
  kcg_copy_SSP_t_indexed_matrix_element(&outC->SSP_largest_target, &outC->_L26);
}

/* $*************** KCG Version 6.1.3 (build i6) ****************
** FindMax_of_all_t_cats.c
** Generation date: 2015-04-06T19:13:00
*************************************************************$ */
