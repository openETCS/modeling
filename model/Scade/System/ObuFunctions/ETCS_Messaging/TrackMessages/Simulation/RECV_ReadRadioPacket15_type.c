...
TypeUtils _SCSIM_array_int_200_200_Utils = {array_int_200_200_to_string,
	check_array_int_200_200_string,
	string_to_array_int_200_200,
	is_array_int_200_200_allow_double_convertion,
	0,
	compare_array_int_200_200_type,
	get_array_int_200_200_signature,
	get_array_int_200_200_filter_utils,
	array_int_200_200_filter_size,
	array_int_200_200_filter_values};
TypeUtils _SCSIM_CompressedBaliseData_Utils = {CompressedBaliseData_to_string,
	check_CompressedBaliseData_string,
	string_to_CompressedBaliseData,
	is_CompressedBaliseData_allow_double_convertion,
	CompressedBaliseData_to_double,
	compare_CompressedBaliseData_type,
	get_CompressedBaliseData_signature,
	get_CompressedBaliseData_filter_utils,
	CompressedBaliseData_filter_size,
	CompressedBaliseData_filter_values};
 ...
 ** array_int_200_200
 ****************************************************************/
void* array_int_200_200_projection(void** pValues, int nIndex) {
	return &((*(array_int_200_200*)pValues)[nIndex]);
}

const char * array_int_200_200_to_string(const void* pValue) {
	return (char*) pSimulator->m_pfnArrayToString((const void**)pValue, array_int_200_to_string, 200, array_int_200_200_projection);
}

int compare_array_int_200_200_type(int* pResult, const char* toCompare, const void* pValue) {
	return pSimulator->m_pfnArrayComparison(pResult, toCompare, (const void**)pValue, compare_array_int_200_type , 200, array_int_200_200_projection);
}
...
int string_to_array_int_200_200(const char* strValue, void* pValue) {
	static array_int_200_200 rTemp;
	int nResult = pSimulator->m_pfnArrayFromString(strValue, (void**)(void*)&rTemp, string_to_array_int_200, 200, array_int_200_200_projection);
	if (nResult == 1)
		kcg_copy_array_int_200_200(&(*((array_int_200_200*)pValue)), &(rTemp));
	return nResult;
}

const char * get_array_int_200_200_signature() {
	return pSimulator->m_pfnArraySignature(get_array_int_200_signature, 200);
}

FilterUtils get_array_int_200_200_filter_utils(const char* strFilter, void* pValue) {
	return pSimulator->m_pfnGetArrayFilterUtils(&_SCSIM_array_int_200_Utils, strFilter, (void**)pValue, 200, array_int_200_200_projection);
}

int check_array_int_200_200_string(const char* strValue) {
	static array_int_200_200 rTemp;
	return string_to_array_int_200_200(strValue, &rTemp);
}
 ...
 ****************************************************************/
struct SimTypeVTable* pSimCompressedBaliseDataVTable;
const char * CompressedBaliseData_to_string(const void* pValue) {
	if (pSimCompressedBaliseDataVTable != 0 && pSimCompressedBaliseDataVTable->m_pfnGetConvInfo(SptString, SptNone) == 1)
		return *(char**)pSimCompressedBaliseDataVTable->m_pfnToType(SptString, pValue);
	return array_int_200_to_string(pValue);
}

int string_to_CompressedBaliseData(const char* strValue, void* pValue) {
	if (pSimCompressedBaliseDataVTable != 0 && pSimCompressedBaliseDataVTable->m_pfnGetConvInfo(SptNone, SptString) == 1) {
		static CompressedBaliseData rTemp;
		int nResult = pSimCompressedBaliseDataVTable->m_pfnFromType(SptString, (const void*)&strValue, &rTemp);
		if (nResult == 1)
			kcg_copy_array_int_200(&(*((CompressedBaliseData*)pValue)), &(rTemp));
		return nResult;
	}
...
int is_CompressedBaliseData_allow_double_convertion() {
	if (pSimCompressedBaliseDataVTable != 0) {
		int nConvertionAllowed = 0;
		nConvertionAllowed |= pSimCompressedBaliseDataVTable->m_pfnGetConvInfo(SptNone, SptLong) == 1;
		nConvertionAllowed |= pSimCompressedBaliseDataVTable->m_pfnGetConvInfo(SptNone, SptShort) == 1;
		nConvertionAllowed |= pSimCompressedBaliseDataVTable->m_pfnGetConvInfo(SptNone, SptDouble) == 1;
		nConvertionAllowed |= pSimCompressedBaliseDataVTable->m_pfnGetConvInfo(SptNone, SptFloat) == 1;
		return nConvertionAllowed;
	}
	return is_array_int_200_allow_double_convertion();
}

int CompressedBaliseData_to_double(double * nValue, const void* pValue) {
	if (pSimCompressedBaliseDataVTable != 0) {
		if (pSimCompressedBaliseDataVTable->m_pfnGetConvInfo(SptNone, SptLong) == 1)
			*nValue = (double)(*(long*)pSimCompressedBaliseDataVTable->m_pfnToType(SptLong, pValue));
		else if (pSimCompressedBaliseDataVTable->m_pfnGetConvInfo(SptNone, SptShort) == 1)
			*nValue = (double)(*(int*)pSimCompressedBaliseDataVTable->m_pfnToType(SptShort, pValue));
		else if (pSimCompressedBaliseDataVTable->m_pfnGetConvInfo(SptNone, SptDouble) == 1)
			*nValue = (*(double*)pSimCompressedBaliseDataVTable->m_pfnToType(SptDouble, pValue));
		else if (pSimCompressedBaliseDataVTable->m_pfnGetConvInfo(SptNone, SptFloat) == 1)
			*nValue = (double)(*(float*)pSimCompressedBaliseDataVTable->m_pfnToType(SptFloat, pValue));
		else
			return 0;
		return 1;
	}
...
int check_CompressedBaliseData_string(const char* strValue) {
	static CompressedBaliseData rTemp;
	return string_to_CompressedBaliseData(strValue, &rTemp);
}
