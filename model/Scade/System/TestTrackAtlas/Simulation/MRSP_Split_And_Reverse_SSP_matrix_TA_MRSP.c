...
void MRSP_Split_And_Reverse_SSP_matrix_reset_TA_MRSP(
  outC_MRSP_Split_And_Reverse_SSP_matrix_TA_MRSP *outC)
{
  kcg_int i;
  
  for (i = 0; i < 11; i++) {
    /* 1 */ MRSP_SplitAndReverseSSP_LOOP1_reset_TA_MRSP(&outC->Context_1[i]);
  }
}
  ...
  outC_MRSP_Split_And_Reverse_SSP_matrix_TA_MRSP *outC)
{
  ...
  for (i = 0; i < 11; i++) {
    /* 1 */
    MRSP_SplitAndReverseSSP_LOOP1_TA_MRSP(&outC->_L1[i], &outC->Context_1[i]);
