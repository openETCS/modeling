...
void MoveSSPSectionsAbsolute_reset_TA_Lib_internal(
  outC_MoveSSPSectionsAbsolute_TA_Lib_internal *outC)
{
  ...
  for (i = 0; i < 50; i++) {
    /* 1 */
    MoveSSPSectionsAbsolute_Loop_reset_TA_Lib_internal(&outC->Context_1[i]);
  }
}
  ...
  outC_MoveSSPSectionsAbsolute_TA_Lib_internal *outC)
{
  ...
  for (i = 0; i < 50; i++) {
    kcg_copy_StaticSpeedProfile_t_TrackAtlasTypes(&tmp, &outC->_L4);
    /* 1 */
